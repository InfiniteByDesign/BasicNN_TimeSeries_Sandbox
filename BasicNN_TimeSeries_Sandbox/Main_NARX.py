...
with graph.as_default():
    ...
    x_train     = x_training_NARX_batches
    x_test      = x_test_batches                    # The NARX testing data will have to be generated dynamically because it depends on the model output
    ...
    numInputs = x_train.shape[2]                    # Number of inputs depends on the number of samples in the NARX dataset
    ...
    with tf.Session() as sess:
        ...
        # Create index and join all the batches
        maxDelay = max(cfg.numInputDelays,cfg.numOutputDelays)
        x_test = np.reshape(x_test,(x_test.shape[0]*x_test.shape[1],x_test.shape[2]))
        y_test = np.reshape(y_test,(y_test.shape[0]*y_test.shape[1],y_test.shape[2]))

        # Initialize the output delay with only the requred number of previous outputs
        y_test = y_test[0:cfg.numOutputDelays+1,:]

        # Loop through the remaining test values
        for i in range(maxDelay+1,x_test.shape[0]): 
            input = func.create_NARX_dataset(x_test[i-maxDelay-1:i,:],y_test[i-maxDelay-1:i,:],cfg.numInputDelays,cfg.numOutputDelays)
            pred = mlp.multiplayer_perceptron_test(sess,cfg,summary_writer,x,keep_prob,input,[input.shape[0], numOutputs],predictions)
            y_test = np.append(y_test, pred, axis=0)
            
        # Display the results
        actual = np.reshape(y_Interval_testing_batches,(-1,y_test.shape[1]))
        interp = np.reshape(y_Interpolate_testing_batches,(-1,y_test.shape[1]))
        func.plot_test_data(0,actual,y_test)
        func.plot_test_data(0,interp,y_test)
